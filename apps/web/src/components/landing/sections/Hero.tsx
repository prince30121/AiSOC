...
export function Hero() {
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="absolute inset-0 -z-10 bg-landing-grad-hero opacity-90"
      />
      <AnimatedGridPattern
      ...
      <div className="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8 lg:pb-28">
        <div className="mx-auto max-w-3xl text-center">
          <p
            className={cn(
              'inline-flex items-center gap-2 rounded-full border border-surface-border bg-surface-raised/60 px-3 py-1 text-xs font-medium text-fg-muted backdrop-blur-sm',
              'animate-fade-in-up',
            )}
          ...
          >
            <span
              aria-hidden="true"
              className="inline-block h-1.5 w-1.5 rounded-full bg-brand-400 shadow-[0_0_0_2px_rgba(59,130,246,0.25)]"
            />
          ...
          <h1
            id="hero-heading"
            className="mt-6 font-sans text-4xl font-bold leading-[1.1] tracking-tight text-fg-primary sm:text-5xl lg:text-[64px] lg:leading-[1.05] lg:tracking-[-0.02em]"
          >
            <TextGenerateEffect
              words="Detect. Triage."
              staggerDelay={0.06}
          ...
          <Link
            href="#demo"
            className="group mt-5 inline-flex items-center gap-2 text-sm font-medium text-fg-muted transition-colors duration-200 ease-landing-out-quart hover:text-fg-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-300 focus-visible:ring-offset-2 focus-visible:ring-offset-surface-base motion-safe:animate-fade-in-up"
        ...
        >
          {PROOF_CHIPS.map((chip) => (
            <li
              key={chip}
              className="inline-flex items-center rounded-full border border-surface-border bg-surface-raised/40 px-3 py-1.5 text-xs font-medium text-fg-muted shadow-[0_0_0_1px_rgba(148,163,184,0.04)_inset] backdrop-blur-sm"
            >
              {chip}
            </li>
          ))}
