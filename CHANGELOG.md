...
### v8.0 foundation — landing page, agent reasoning, realtime graph, GTM

A coherent wave of changes that together close out the v8.0 architectural
foundation and the launch surface around it. Highlights:

- **T6.5 marketing landing page (`apps/web/src/app/page.tsx`).** Full
  composition lands: sticky nav, animated hero, social-proof strip,
  problem framing, four-agent pipeline diagram, embedded product demo,
  differentiation pillars, feature grid, connectors marquee, benchmark
  big-numbers band, deploy-options matrix, open-source moment,
  testimonials, pricing teaser, FAQ accordion, final CTA, and footer.
  Backed by vendored Aceternity + MagicUI primitives, `apps/docs`-aligned
  design tokens, and JSON-LD + performance hardening for first paint.
- **Realtime graph end-to-end.** Go-side broadcaster + Kafka consumer in
  `services/ingest/internal/graph_ws/`, an auth-binding API proxy in
  `services/api/app/api/v1/endpoints/graph_ws.py`, and the
  `useGraphWebSocket` hook + `RealtimeGraph.tsx` Cytoscape canvas in
  `apps/web`. Tenant ID is rebound server-side from the session, so the
  browser cannot cross-subscribe.
- **Agent reasoning topology.** `ContextBundle` pre-fetch
  (`services/agents/app/context/bundle.py`) gathers graph neighbourhood,
  recent telemetry, MITRE mappings, and prior-investigation context once
  per turn; the LangGraph orchestrator now fans the four sub-agents out in
  parallel (`services/agents/app/orchestrator/graph.py`) and the LLM
  input contract (T2.3) fails closed on raw OCSF entering a user message.
- **Aggregator endpoints + `/hunt`.** `GET /api/v1/insights`,
  `GET /api/v1/permissions/effective`, `GET /api/v1/attack-chains`, and
  the natural-language `/hunt` surface (with curated ES|QL / SPL / KQL
  templates so the agent never writes raw queries) plus a JSON-rule
  business-context engine with Monaco editor in the console.
- **Benchmark scoreboard + telemetry.** Token + USD telemetry on every
  eval run, SVG charts in-tree, the public weekly scoreboard at
  `/docs/benchmark-scoreboard` (substrate rows visually separated from
  wet-eval rows), and the T5.5 weekly wet-eval CI workflow that appends
  real rows on top.
- **Managed-beta onboarding stack.** Waitlist signup model + endpoint,
  tenant provisioner service + migration, and an operations doc for
  `app.aisoc.dev` under `apps/docs/docs/operations/`.

Ten more first-party connectors land alongside the above: Tines, Torq,
Falco, PagerDuty, Confluence Audit, Cloudflare Zero Trust, Sysdig, Vault,
Snowflake, with six more in scaffold state behind feature flags. The
five-tier severity ladder (`info | low | medium | high | critical`) is
preserved end-to-end on all new connectors.
